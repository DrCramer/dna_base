...
export function ExportPage() {
  ...
  const query = useMemo(() => [q, partyRange, objectNos, stage, year, onlyProblematic ? 'проблемные' : ''].filter(Boolean).join(' '), [objectNos, onlyProblematic, partyRange, q, stage, year])
    ...
    <div className="page">
      <PageHeader
        ...
        actions={(
          <>
            ...
            <a className="primary download" href={api.exportRegistryUrl(query)}><FileDown size={18} />Скачать Excel</a>
          </>
        )}
      />
      ...
      <section className="section">
        ...
        <div className="export-mode-grid">
          {exportModes.map((item) => (
            <button key={item.id} className={`mode-card${mode === item.id ? ' active' : ''}`} onClick={() => setMode(item.id)}>
      ...
      <section className="section">
        ...
        <div className="filter-panel">
          ...
          <label>Партии
            <input value={partyRange} onChange={(event) => setPartyRange(event.target.value)} placeholder="160, 157 или 150-160" />
          </label>
          <label>Диапазон / номера объектов
            <input value={objectNos} onChange={(event) => setObjectNos(event.target.value)} placeholder="3303-1, 3304-1..." />
          </label>
          <label>Поиск
