...
- Создание, сохранение и A4-печать единых протоколов Выделение / RT / PCR / Форез с несколькими 96-луночными плашками.
...
В разделе `Протоколы -> Создать` выберите год и несколько партий, отметьте объекты, заполните четыре этапа прямо в документе и сохраните черновик. После финализации документ открывается из `Протоколы -> Сохранённые` по зафиксированному snapshot; дальнейшее редактирование создаёт новую ревизию. Доступны A4 landscape preview, печать и `Скачать Excel`.
...
Playwright smoke для раздела протоколов (приложение должно быть запущено на порту `4001`):
